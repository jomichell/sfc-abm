# This code reproduces the results in this paper:
# https://www.postkeynesian.net/working-papers/1412/
#
# The code contains errors in the accounting for
# dividend payments and for bankruptcies. The simulation
# figures included in the paper therefore reflect these
# errors, although the results are not meaningfully
# different once the errors are corrected.
...
def solve_aggregate(year):
    ...
    year and returns the results in a dictionary
    """
    ...
    f_r = llambda * f_n     # retained profits
    f_d = f_n - f_r         # distributed profits

    l = ts['l'][year]     # firms sector loan demand

    m_f = ts['m_f'][year-1] + f_r + \
        (l - ts['l'][year-1]) - i       # firms' banks deposits (residual)
    
    y_hr = wb + (r_m * ts['m_h'][year-1]) + f_d # household sector total income
    m_h = ts['m_h'][year-1] + y_hr - c  # households deposits 
...
for year in years[1:]:
    ...
    for firm in firm_ts:
        ...
        # REPLACED THIS
        # lq_e = firm[m_f][year-1] + firm[f_r][year-1] - firm[i_d][year]

        # WITH EXPECTED INCOME - CURRENT WAGE BILL
        ...
        #firm[l_d][year] = firm[l_d][year-1] + lq_d - lq_e
        ...
        if DEBUG:
            ...
            """.format(
                ...
                lq_d,
                lq_e,
                firm[l_d][year] - firm[l_d][year-1],
                firm[l_d][year]         
                ))

        
    #
    # put the totals into the macro time-series
    #

    print("""Inserting aggregates from micro:
    y_s: {:f}
    m  : {:f}
    i: {:f}
    wb: {:f}    
    ...
    print(""" Summary values from micro:
    Dl_d : {:f}
    f_lb : {:f}
    lq_d : {:f}     
    lq_e : {:f}
    ...
    # Set up parameter shocks here
    if year == 1100:
        print("*** PERFORMING SHOCKS at {} ***".format(year))
        #r_l_bar = 0.06
        #alpha1 == 0.9
        #zeta = 0.9
        #tau = 0.3
    ...
    print("*** Performing macro solution analysis ****")
    ...
    """.format(
        ...
        sol['wb'],
        sol['y_hr'],
        sol['m_h'] - ts['m_h'][year-1],
        sol['m_f'] - ts['m_f'][year-1],
        sol['l'] - ts['l'][year-1],      
        sol['m_h'],
        sol['m_f'],
        sol['m_t'],
    ...
    print("*** Performing post-stage micro analysis ****")
    ...
    for firm in firm_ts:
        ...
        if firm['m_f'][year] < 0:
            ...
            ts['m_h'][year] -= firm['l'][year]
            ts['m_t'][year] -= firm['l'][year]
    ...
    print("Check: macro k: %0.2f micro k: %0.2f" % (sol['k'], agg_k))
    print("Check: macro y: %0.2f micro y: %0.2f" % (sol['y'], agg_y))
    print("Check: macro iv: %0.2f micro iv: %0.2f" % (sol['iv'], agg_iv))
    print("Check: macro f_t: %0.2f micro f_t: %0.2f" % (sol['f_t'], agg_f_t))
    print("Check: macro m_f: %0.2f micro m_f: %0.2f" % (sol['m_f'], agg_m_f))
    print("Check: macro r: %0.2f micro r: %0.2f" % (sol['r'], agg_r))
    print("Check: macro wb_d: %0.2f micro wb_d: %0.2f" % (sol['wb'], agg_wb))
